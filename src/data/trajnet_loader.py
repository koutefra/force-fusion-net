import json
from data.base_loader import BaseLoader
from entities.raw_data import RawDataCollection, RawSceneData
from entities.vector2d import Point2D
from typing import Optional

class TrajnetLoader(BaseLoader):
    def load_scene_by_ids(self, scene_ids: set[int]) -> RawDataCollection:
        return self.load_scenes_by_ids(scene_ids=scene_ids)

    def load_all_scenes(self) -> RawDataCollection:
        return self.load_all_scenes()

    def _load_scenes(self, scene_ids: Optional[set[int]]) -> RawDataCollection:
        scenes = []
        tracks = []
        with open(self.path, 'r') as file:
            for line in file:
                data = json.loads(line)
                if 'scene' in data and (not scene_ids and data['scene']['id'] in scene_ids):
                    scene_data = data['scene']
                    scenes.append(RawSceneData(
                        id=scene_data['id'],
                        focus_person_id=[scene_data['p']],
                        goal_positions={},
                        start_frame_number=scene_data['s'],
                        end_frame_number=scene_data['e'],
                        fps=scene_data['fps'],
                        tag=scene_data.get('tag')
                    ))
                elif 'track' in data:
                    track_data = data['track']
                    if scene_ids:
                        for scene in scenes:
                            if scene.start_frame_number <= track_data['f'] <= scene.end_frame_number:
                                tracks.append(RawSceneData(
                                    frame_number=track_data['f'],
                                    id=track_data['p'],
                                    type="person",
                                    position=Point2D(x=track_data['x'], y=track_data['y'])
                                ))
                    else:
                        tracks.append(RawSceneData.TrackData(
                            frame_number=track_data['f'],
                            person_id=track_data['p'],
                            x=track_data['x'],
                            y=track_data['y']
                        ))
        return RawSceneData(scenes, tracks)